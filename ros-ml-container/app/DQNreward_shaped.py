...
class fhtw3dof_episodic(gym.Wrapper):
    def __init__(self, env):
        ...
        self.max_goalstate_distance = 0.05                      # maximum distance between goal and robot tcp for the goal to count as reached
        self.render=False                                       # render to rviz
        self.env.P = self.env.reset_rewards(env.P, 0, env.nS)
        self.goalstate = 0
        self.goalstate = self.generate_goalstate()
        self.last_goalstate_distance = self.get_goalstate_distance(self.env.s)    # keep track of last goalstate distance for reward shaping
    ...
    def step(self, action):
        ...
        # store goalstate distance and auxiliary reward values
        distance = new_state[0][6]
        aux_reward = 0.001
        # shape reward in case the step was neutral
        if reward == 0:
            if distance < self.last_goalstate_distance: reward = aux_reward
            elif distance > self.last_goalstate_distance: reward = -aux_reward
        # keep track of last goalstate distance
        self.last_goalstate_distance = distance
...
class DQN_Agent:
    def __init__(self, env):
        ...
        self.epsilon_decay = 0.9992             # rate at which epsilon is discounted
