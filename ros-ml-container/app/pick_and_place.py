...
- Interpolate between goalstate and grasp point, in order to ensure that the robot ends up there

- Bleibende Regelabweichung (besonders an j2) (DONE)!!

- do not call init_node in the env by default (DONE)

- automatically adjust distance when no state is found (NOT NECESSARY)

- Debug higher joint resolution (Woks, DONE)

- Fix find nearest goalstate (DONE)

- Fix goal height (IN VISION SYSTEM)

- Make Serial Gripper optional in hebi pub (DONE)
...
while not rospy.is_shutdown():
    pt = get_object_position()
    j3, j2, j1 = inaccuracy_compensation.getJointstate(pt.point.x, pt.point.y, pt.point.z)
    goto_jointstate(j3, j2, j1, env)
    rate.sleep()
"""
...
env.render_state(home_state)
hebi_pib.publish_pose_to_robot()
"""
...
r = apply_policy(place_policy, place_state, env)
pt = env.TCP_Positions[place_state]
j3, j2, j1 = inaccuracy_compensation.getJointstate(pt.x, pt.y, pt.x)
goto_jointstate(j3, j2, j1, env)
hebi_pub.publish_trajectory_to_robot()
time.sleep(20)
...
pt = env.TCP_Positions[home_state]
j3, j2, j1 = inaccuracy_compensation.getJointstate(pt.x, pt.y, pt.x)
goto_jointstate(j3, j2, j1, env)
